...
local function passThrough(key) return key end
local function replaceDots(key) return string.gsub(key, "%.", ",") end
...
betterContacts.setOptions = function(newOptions)
    if type(newOptions) == "table" then
        ...
        for key, newValue in pairs(newOptions) do
            ...
            elseif oldType == "nil" then
                ...
                local message = string.format("Die Option '%s' muss vom Typ %s sein, aber ist vom Typ %s", key,
                                              oldType, newType)
                error(message, 2)
            end
        end
    ...
    if options.deprecatedUseGlobal then
        ...
        if options.varnameTrackID ~= "" then params = params .. "," .. options.varnameTrackID end
...
mt.__index = function(self, key) return parseKey(self, key) or queryOldIndex(self, key) end
...
setmetatable(betterContacts, {__call = function(_, newOptions) return betterContacts.setOptions(newOptions) end})
