...
local function createTrackState()
    return { registered = true, trainName = "" }
end
...
local function create(simulator, globals)
    ...
    local signalsTrainCount = state.signalQueues.countBySignal
    local signalsTrainNames = state.signalQueues.namesBySignal
    local hook = state.rollingstock.hookByName
    local hookGlue = state.rollingstock.hookGlueByName
    local eepdata = state.persistence.dataSlots
    local structureAxis = state.structureRuntime.axisByStructure
...
local function getOrCreateRollingStockEntry(rollingStockName)
    return select(1, getRollingStockTrainContext(rollingStockName)) or getOrCreateImplicitRollingStockTrain(rollingStockName)
end
...
function Runtime.simulatePlaceTrainOnRoadTrack(trackId, zugname)
    Store.ensurePath(state, { "trains", zugname }, createTrainState).onLayout = true
...
function Runtime.simulateAddTrainToTrainyard(depotId, trainName, position, status)
    ...
    Store.ensurePath(state, { "trains", trainName }, createTrainState).onLayout = entry.status ~= 1
end

function Runtime.callEEPSetSignal(signalId, signalStellung, informiereEepOnSignal)
    assert(signalId > 0)
    Store.ensurePath(state, { "signals", signalId }, createEmptyTable).stellung = signalStellung
    return 1
end
...
function Runtime.callEEPSetSwitch(switchId, switchPosition, activateEEPOnSwitch)
    Store.ensurePath(state, { "switches", switchId }, createEmptyTable).stellung = switchPosition
    return 1
end

--- Liefert die aktuelle Stellung der Weiche x
function Runtime.callEEPGetSwitch(switchId) return switches[switchId] and switches[switchId].stellung or 2 end
...
function Runtime.callEEPSetColourFilter(farbton, saettigung, helligkeit, kontrast) end
-------------------
-- Neu ab EEP 11 --
-------------------
...
-- @param rsName Name des Rollmaterial,
-- @param kupplungsStatus 1-Kupplung aktiv, 2-Kupplung inaktiv, 3-Wagen angekoppelt(nurGet),
function Runtime.callEEPRollingstockGetCouplingRear(rsName)
...
-- @param rsName Name des Rollmaterial,
-- @param kupplungsStatus 1-Kupplung aktiv, 2-Kupplung inaktiv, 3-Wagen angekoppelt(nurGet),
function Runtime.callEEPRollingstockGetCouplingFront(rsName)
...
--------------------------------------------------------------------------
function Runtime.callEEPRollingstockSetAxis(rsName, achse, stellung) end

--- Gibt die Stellung der Achse am Rollmaterial zurueck
-- z.B.: EEPRollingstockSetAxis("Bekohlungskranbr?cke 1", "Drehung links", 50)
-- @param rsName Name des Rollmaterials als String
-- @param achse Name der Achse
function Runtime.callEEPRollingstockGetAxis(rsName, achse) end
...
function Runtime.callEEPLoadData(slot) return ((eepdata[slot] or eepdata[slot] == false) and true or false), eepdata[slot] end
...
function Runtime.callEEPSaveData(slot, data)
    eepdata[slot] = data
    return true
end
...
--- Rauch abfreagen
-- @param immoName Name der Immobilie als String.
function Runtime.callEEPStructureGetSmoke(immoName) end
...
function Runtime.callEEPStructureSetLight(name, onoff)
    getOrCreateStructureState(name).light = onoff
end

--- Licht abfragen
-- @param immoName Name der Immobilie als String.
function Runtime.callEEPStructureGetLight(name)
    local strippedName = stripImmoName(name)
...
--- Feuer abfragen
-- @param immoName Name der Immobilie als String.
function Runtime.callEEPStructureGetFire(immoName) end
...
function Runtime.callEEPStructureSetAxis(immoName, achse, stellung)
...
function Runtime.callEEPStructureSetPosition(immoName, posX, posY, posZ) end
...
function Runtime.callEEPStructureSetRotation(immoName, rotX, rotY, rotZ) end
...
function Runtime.callEEPSetTrainRoute(trainName, route)
    Store.ensurePath(state, { "trains", trainName }, createTrainState).route = route
    return true
end
...
function Runtime.callEEPSetTrainLight(trainName, onoff, quelle)
    local train = Store.ensurePath(state, { "trains", trainName }, createTrainState)
...
-- @param trainName Name des Zuges
-- @param onoff true: ein, false: aus
function Runtime.callEEPSetTrainSmoke(trainName, onoff)
    Store.ensurePath(state, { "trains", trainName }, createTrainState).smoke = onoff == true
    return true
end
...
function Runtime.callEEPSetTrainHorn(trainName, onoff)
    Store.ensurePath(state, { "trains", trainName }, createTrainState).horn = onoff == true
    return true
end
...
function Runtime.callEEPSetTrainCouplingFront(trainName, kupplungOn)
...
function Runtime.callEEPSetTrainHook(trainName, gueteran)
    Store.ensurePath(state, { "trains", trainName }, createTrainState).hook = gueteran == true
    return true
end
...
function Runtime.callEEPSetCamera(camType, camName) return true end

--- Waehlen einer Kameraperspektive
-- @param camPosition Tasten 1 - 9 fuer die Kameraposition
-- @param trainName Name des Zuges
-- @return true, wenn die Kamera existiert
function Runtime.callEEPSetPerspectiveCamera(camPosition, trainName)
...
function Runtime.callEEPGetTrainFromTrainyard(depotId, trainName, trainNumber)
    ...
    entry.status = 0
    Store.ensurePath(state, { "trains", entry.name }, createTrainState).onLayout = true
    Runtime.emitOnTrainExitTrainyard(depotId, entry.name)
    return true
end
...
function Runtime.callEEPPutTrainToTrainyard(depotId, trainName)
    ...
    entry.status = 1
    Store.ensurePath(state, { "trains", trainName }, createTrainState).onLayout = false
...
function Runtime.callEEPShowInfoStructure(immoName, onOff) end
...
-- @param onOff true: einschalten
function Runtime.callEEPShowInfoSignal(signalId, onOff) assert(type(signalId) == "number", "Need 'signalId' as number") end

--- Setzen des Tipp-Textes einer Immobilie
-- @param switchId Name der Immobilie als String.
-- @param text Text fuer die Anzeige
function Runtime.callEEPChangeInfoSignal(signalId, text) assert(type(signalId) == "number", "Need 'signalId' as number") end
...
--
function Runtime.callEEPGetRollingstockItemsCount(zugverband)
...
-- @param signalId ID des Signals
--
function Runtime.callEEPGetSignalTrainsCount(signalId) return signalsTrainCount[signalId] or 0 end
...
function Runtime.callEEPGetTrainyardItemStatus(depotId, zugverband, position)
...
function Runtime.callEEPStructureGetPosition(name)
...
function Runtime.callEEPStructureSetTagText(name, tag)
    getOrCreateStructureState(name).tag = tag
    return true
end
...
--- * Rueckgabewert ist true, wenn die Ausfuehrung erfolgreich war, sonst false.
function Runtime.callEEPRollingstockSetTagText(name, tag)
...
function Runtime.callEEPSignalSetTagText(id, tag)
    Store.ensurePath(state, { "signals", id }, createEmptyTable).tag = tag
    return true
end

function Runtime.callEEPSignalGetTagText(id) return true, signale[id] and signale[id].tag or nil end

function Runtime.callEEPSwitchSetTagText(id, tag)
    Store.ensurePath(state, { "switches", id }, createEmptyTable).tag = tag
    return true
end

function Runtime.callEEPSwitchGetTagText(id) return true, switches[id] and switches[id].tag or nil end

function Runtime.callEEPGoodsSetTagText(name, tag)
    Store.ensurePath(state, { "goods", name }, createEmptyTable).tag = tag
    return true
end
...
function Runtime.callEEPStructureSetTextureText(name, flaeche, text)
    return setTextureText(getOrCreateStructureState(name), flaeche, text)
end

function Runtime.callEEPStructureGetTextureText(name, flaeche)
...
function Runtime.callEEPSignalSetTextureText(id, flaeche, text)
    return setTextureText(Store.ensurePath(state, { "signals", id }, createEmptyTable), flaeche, text)
end

function Runtime.callEEPSignalGetTextureText(id, flaeche) return getTextureText(signale[id], flaeche) end

function Runtime.callEEPGoodsSetTextureText(name, flaeche, text)
    return setTextureText(Store.ensurePath(state, { "goods", name }, createEmptyTable), flaeche, text)
end

function Runtime.callEEPGoodsGetTextureText(name, flaeche) return getTextureText(state.goods[name], flaeche) end

function Runtime.callEEPRailTrackSetTextureText(id, flaeche, text) return setTrackTextureText("rail", id, flaeche, text) end

function Runtime.callEEPRailTrackGetTextureText(id, flaeche) return getTrackTextureText("rail", id, flaeche) end

function Runtime.callEEPRoadTrackSetTextureText(id, flaeche, text) return setTrackTextureText("road", id, flaeche, text) end

function Runtime.callEEPRoadTrackGetTextureText(id, flaeche) return getTrackTextureText("road", id, flaeche) end

function Runtime.callEEPTramTrackSetTextureText(id, flaeche, text) return setTrackTextureText("tram", id, flaeche, text) end

function Runtime.callEEPTramTrackGetTextureText(id, flaeche) return getTrackTextureText("tram", id, flaeche) end

function Runtime.callEEPAuxiliaryTrackSetTextureText(id, flaeche, text)
...
function Runtime.callEEPActivateCtrlDesk(GBSname) return true end
...
-- @param status true = an, false = aus
-- @return ok Rueckgabewert ist true wenn die Ausfuehrung erfolgreich war, sonst false
function Runtime.callEEPRollingstockSetHook(rollingstockName, status)
...
function Runtime.callEEPRollingstockGetHook(rollingstockName) return true, hook[rollingstockName] and 1 or 0 end
...
function Runtime.callEEPGetCameraPosition() return true, camera.PosX or 0, camera.PosY or 0, camera.PosZ or 0 end
...
function Runtime.callEEPGetCameraRotation() return true, camera.RotX or 0, camera.RotY or 0, camera.RotZ or 0 end
