...
export class TableDataSource<T> extends DataSource<T> {
  filterPredicate: (data: any, filterText: any) => boolean;
  private data: T[];
  ...
  connect(): Observable<T[]> {
    ...
    this.filterSubscription = this.filter.subscribe((filterText) => {
      this.filterText = filterText && filterText.length > 0 ? filterText : null;
    });

    const dataMutations = [this.data$, this.filter, this.sort.sortChange];
  ...
  private getFilteredData(allData: T[]) {
    return this.filterText ? allData.filter((data) => this.filterPredicate(data, this.filterText)) : allData;
  }
}
